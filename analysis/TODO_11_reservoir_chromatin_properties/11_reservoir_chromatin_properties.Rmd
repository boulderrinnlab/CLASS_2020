...
```{r} 
# Loading in common features to make a data frame of histone marks

# Peak occurence of promoters and DBPs bound
...
peak_occurrence_df <- read_csv("../
                               
# This will come from 10 with nascent TPM adding
                               
                               08_defining_reservoirs/results/08_peak_occurrence_df_promoter_types.csv")
...
reservoir_promoters <- rtracklayer::import ("../ACTIVE_09_reservoir_binding_properties/results/k562_reservoir_promoters_info.csv")
...
peak_files <- data.frame("file" = list.files("/Shares/rinn_class/data/CLASS_2020/analysis", full.names = TRUE, pattern = ".bed"),
                         "histone_mark" = c("H3K27me3", "H3K27ac", "H3K4me1", "H3K9me1", "H3K9me3"))
# Read in peaks
se_hg19 <- rtracklayer::import("/Shares/rinn_class/data/CLASS_2020/analysis/07_reservoirs/data/K562.bed")
hmark_peaks <- lapply(peak_files$file, read.table)
names(hmark_peaks) <- peak_files$histone_mark
hmark_granges <- lapply(hmark_peaks, function(peaks) {
  GRanges(seqnames = peaks$V1,
