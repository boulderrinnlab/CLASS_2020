...
k27me_ov <- read("data/ENCFF031FSF.bed")
k27ac_ov <- rtracklayer::import("data/ENCFF038DDS.bed")

findOverlaps(peak_occurrence_df, k27me_df)


peak_occurrence_df <- read.csv("../10_reservoir_nascent_txn/results/peak_occurrence_df_nascent_added.csv")
k27me_df <- read.table("data/ENCFF031FSF.bed")
k27ac_df <- read.table("data/ENCFF038DDS.bed")

write.csv(k27me_df, "")



peak_occurrence_df <- merge(peak_occurrence_df,
...
makeGRangesFromDataFrame(peak_occurrence_df,
                         )
...
hmark_granges <- lapply(hmark_peaks, function(peaks) {
 ...
 k27me3_ov <- findOverlaps(hmark_granges$H3K27me3@ranges, promoters@ranges)
 k27ac_ov <- findOverlaps(hmark_granges$H3K27ac@ranges, promoters@ranges)
 
 write.csv(k27me3_ov, "results/k27me3_ov.csv")
 write.csv(k27ac_ov, "results/k27ac_ov.csv")
})
...
plot(density(peak_occurrence_df$nascentTpm))
```
...
use promoters from 01 as gRanges and granges of histone marks to make peak occurence df

merge into peak_occurence df
