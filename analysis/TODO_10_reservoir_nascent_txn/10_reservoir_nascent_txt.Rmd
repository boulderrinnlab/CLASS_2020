...
# Using Rsubread to quantify the nascent epxression over promoter windows (3kb +/- start)
```{r}

# Rsubread to make counts on features.

#TODO Does not run I have the BAMs
counts <- featureCounts(c("data/516.sorted.bam", "data/829.sorted.bam", annot.ext = "results/gencode_v32_promoters_6kb.SAF", isPairedEnd = TRUE, nthreads = 16))

# This runs
counts <- featureCounts(c("data/516.sorted.bam", "data/829.sorted.bam"))

# This now runs and seems to produce the right file.
