...
round(ecdf(peak_occurrence_df$number_of_tfs)(num_peaks_threshold)*100,1)
```
...
peak_occurrence_df[peak_occurrence_df$number_of_tfs > 7 &
                     peak_occurrence_df$tpm < 0.001, "reservoir"] <- TRUE
table(peak_occurrence_df$reservoir)
```
...
g <- ggplot(peak_occurrence_df %>% filter(reservoir == T),
            aes(x = shared_promoter_type, y = number_of_tfs))
g + geom_boxplot() +
...
expression_window_df <- peak_occurrence_df %>%
  ...
  dplyr::select(number_of_tfs, tpm, reservoir, annotated_superenhancer, shared_promoter_type,
                chr, window_5) %>%
  ...
  summarize(mean_tpm = mean(tpm),
            ...
            total_dbps_bound = sum(number_of_tfs),
            mean_dbps_bound = mean(number_of_tfs),
            n_occupied_promoters = length(which(number_of_tfs > 0)),
            dbps_bound = paste(number_of_tfs, collapse = ";"),
...
ggave("figures/mean_expression_in_5_gene_windows_res_vs_nonres.pdf")
