...
aws s3api list-objects --bucket bchm5631sp2020 > bchm5631_s3_bucket_contents.json
...
tti <- read_csv("targets_to_include.csv") %>%
  dplyr::rename(target = "names.consensus_peaks.") %>%
  dplyr::select(target) %>%
  mutate(include = TRUE)
data_completeness_targets <- merge(data_completeness_targets, tti, all.x = TRUE)
data_completeness_targets <- data_completeness_targets %>%
  filter(include == TRUE)
names(tti)
# data_completeness_targets <- data.frame(target = unique(c(bigwigs$target, 
#                                                           broadpeak$target, 
...
# http://genome.ucsc.edu/cgi-bin/hgTracks?db=hg38&hubUrl=https://bchm5631sp2020.s3.amazonaws.com/trackhub/hub.txt
```
