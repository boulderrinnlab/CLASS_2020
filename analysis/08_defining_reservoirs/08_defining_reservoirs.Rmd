...
## Making conservative set of reservoirs by expanding windows around reservoir to 6kb
## This will further eliminate reservoirs that have a gene within 6Kb that is expressed
```{r}
...
# write_rds(promoter_counts, "promoter_counts.rds")
counts <- promoter_counts$counts %>%
  as.data.frame() %>%
  rownames_to_column("gene_id")

pkdf <- peak_occurrence_df %>% merge(counts)

res <- pkdf %>% filter(reservoir == 1)

really_zero <- pkdf %>% filter(ENCFF625ZBS.bam == 0, ENCFF630HEX.bam == 0)
```
