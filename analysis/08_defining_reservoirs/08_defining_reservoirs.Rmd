...
fl <- list.files("../07_binding_versus_expression/data", pattern = "\\.bam", full.names = T)
fl
promoter_counts <- featureCounts(fl, 
...
table(peak_occurrence_df$conservative_reservoir)
...
g + geom_bar(stat = "identity") + facet_grid(~shared_promoter_type) +
   geom_text(aes(label = count, y = count + 5),
            size = 3)
ggsave("figures/shared_promoters_at_least_one_expressed.pdf")
```
