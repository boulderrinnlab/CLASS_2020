...
#A: Yes it is reasonable, no they probably won't map to centromeres/telomeres etc - but maybe we should mask these regions
...
#A: mask some regions - centromeres, telomeres
...
#sapply applies a function over a list
...
#A: 185 TFs x 2 sets regions = a lot of permutation tests
...
shuffle_peaks <- function (peak_gr, genome) {
  ...
  #make a vector same length as peak_gr with correct chromosome for each observed peak
  ...
  jj <- order(names(nnt))
  #order nnt in chromosome order (if not already)
  nnt <- nnt[jj]
  #same for chromosome lengths
  chrLens <- chrLens[jj]
  #get random start sites for each peak (sample sets random numbers)
  ss <- unlist(sapply(1:length(nnt), function(i) sample(chrLens[i], 
                                                        nnt[i])))
  #keeps peaks the same but changes the start sites
...
#A: right before you call the functions that need random numbers
...
#A: 500 GB maybe - could run on compute nodes instead
# HINT: as.numeric(object.size(peak_list)), to turn bytes in to GB divide by 1e9.

# Inflation rate of one peak list
as.numeric(object.size(permuted_peaks))/as.numeric(object.size(peak_list[1]))

View(p.adjust)

230*(object.size(peak_list))/1e9
...
set.seed(459734)
peak_list_nulls <- lapply(peak_list[5:7], make_permutations, genome = chr_len, nperms = 1000)
