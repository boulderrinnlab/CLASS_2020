...
#I dont think we need an arguement because the consensus file path is the arguement and is part of the file path


# Now we have return a list of all the peaks for each DNA binding protien which is usally our starting point 
...
features <- gtf[which(gtf$transcript_type == "lncRNA")]
...
overlaps <- get_overlapping_peaks(features, peak_list)
...
promoters <- get_promoter_regions(gtf)
```
...
peak_count <- count_peaks_per_feature(features, peak_list)
```
