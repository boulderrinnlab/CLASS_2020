...
#I dont think we need an argument because the consensus file path is the argument and is part of the function


# Now we have returned a list of all the peaks for each DNA binding protein which is usually our starting point 
...
#' this function will intersect the consensus_peaks with gene_body, promoters, mRNA_promoters, lncRNA_promoters, te_family
#' 
#' @param features
...
#import annotated genome as GRanges
gtf <- rtracklayer::import("/Shares/rinn_class/data/genomes/human/gencode/v32/gencode.v32.annotation.gtf")
#just to see what the transcript types are called in gtf
unique(gtf$gene_type) 

#GRanges for mRNA:
#protein_coding_gr <- gtf[which(gtf$gene_type == "protein_coding")]

#create features GRanges with just lncRNAs
lncRNA_gr <- gtf[which(gtf$transcript_type == "lncRNA")]

#now make the function getting overlaps between whatever feature you defined above and all proteins
...
overlaps <- get_overlapping_peaks(lncRNA_gr, peak_list)
...
get_promoter_regions <- function(gtf, biotype, upstream = 2000, downstream = 2000) {
...
proms <- get_promoter_regions(gtf, "lncRNA")
```
...
peak_count <- count_peaks_per_feature(lncRNA_gr, peak_list)

#trying to see if there's stuff in peak_count
ncol(peak_count)
peak_count[130, c(3000, 3005)]
```
...
#is there stuff in promoter_peak_occurence?
ncol(promoter_peak_occurence)
promoter_peak_occurence[100, c(20, 5000, 12000)]
```
...
#subset rmsk_gr for TE class:
rmsk_class <- subset_rmsk(rmsk_gr, "class")
