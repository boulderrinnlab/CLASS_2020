...
# Get the promoter regions of each lncRNA, mRNA, snRNA and snoRNA.

promoter_type <- c("lncRNA", "protein_coding", "miRNA", "snRNA", "snoRNA")
proms <- get_promoter_regions(gencode_gr = gtf, biotype = promoter_type,
                              upstream =3000, downstream = 3000)
df_proms <- data.frame(proms)
...
What sort of RNAs are reservoirs/ghosts?
```{r}
ghost_names <- ghosts$gene_id
ghost_df <- data.frame("ghost_transcript_name" = character(),
                       "ghost_transcript_type" = character()
                       )

for(i in 1:length(ghost_names)){
  gtf_ghost <- unique(gtf[gtf$gene_id == ghost_names[i]])
  gtf_ghost_type <- gtf_ghost$gene_type %>% unique()
  gdf <- data.frame("ghost_transcript_name" = unique(gtf_ghost$gene_id),
                    "ghost_transcript_type" = gtf_ghost_type )
  
  ghost_df <- bind_rows(ghost_df, gdf)
}

## separate out transcripts by gene type
total_ghost_transcript <- length(ghost_df$ghost_transcript_name)

ghost_df_lengths <- data.frame("transcript_type" = character(),
                               "number_of_ghosts" = integer())

for(i in 1:length(promoter_type)){
  transcript_type <- promoter_type[i]
  number_of_ghosts <- length(which(ghost_df$ghost_transcript_type == promoter_type[i]))
  pdff <- data.frame("transcript_type" = transcript_type,
                     "number_of_ghosts" = number_of_ghosts)
  ghost_df_lengths <- bind_rows(pdff, ghost_df_lengths)
}

ghost_gencode <- gencode_gr[gencode_gr$gene_id %in% ghost_df$ghost_transcript_name & gencode_gr$type == "gene"] %>% 
  as.data.frame()

g <- ggplot(ghost_df_lengths, aes(x = transcript_type, y = number_of_ghosts))
g + geom_bar(stat = "identity") + theme(panel.grid = element_blank())
...
for(i in 1:length(num_peaks)) {
  ...
  tdf <- data.frame("tf" = names(num_peaks[i]),
                    ...
                    "diff" = not_ghost_num_overlaps[[i]] - ghost_num_overlaps[[i]])
  
  chisq_res <- bind_rows(chisq_res, tdf)
}

significant_ghosts <- chisq_res %>% filter(chisq_pval != 0)

non_signif_ghosts <- chisq_res %>% filter(chisq_pval == 0)
```
...
Identifying top 10 promoters acting as the reservoirs with most TFs bound to it

```{r}
gtf_df <- as.data.frame(gtf)

gene_identity <- data.frame("gene_name" = gtf_df$gene_name,
                    "gene_id" = gtf_df$gene_id )

# Merge in the expression data into the data frame
peak_occurence <- merge(unique(gene_identity[,colnames(gene_identity)]), peak_occurence_df)

# Defining ghosts here with the gene name
ghosts_with_name <- peak_occurence %>% filter(number_of_tfs > 7, tpm < 0.001)

<<<<<<< HEAD
# Plotting
g5 <- ggplot(ghosts_with_name, aes(number_of_tfs, gene_name))
g5 + geom_point() + xlim (90, 120) + geom_text(aes(label = gene_name), check_overlap = TRUE, hjust = 0, nudge_x = 0.5, size = 2.5) + theme(legend.position = "none", panel.grid = element_blank(), axis.text.y = element_blank())
=======
# Plotting the graph
g5 <- ggplot(ghosts_with_name, aes(gene_name, number_of_tfs))
g5 + geom_point() + ylim (90, 120) + geom_text(aes(label = gene_name), check_overlap = TRUE, vjust = 0, hjust = 0, nudge_y = 1, size = 2.5) + theme(legend.position = "none", panel.grid = element_blank(), axis.text.x = element_blank())
>>>>>>> 3557ad1d4c069a7e5a7837ebaaea6892a230c0ef
```
