...
team_peaks <- import_peaks()
team_peaks



# Now we have returned a list of all the peaks for each DNA binding protien which is usally our starting point 
...
Above we have a GRanges object with our DNA binding name and all the corresponding consensus peaks
...
protein_coding <- gtf[which(gtf$gene_type == "protein_coding")]
features <- gtf[gtf$gene_type == "protein_coding" & gtf$type == "gene"]
...
overlapping_peaks <- get_overlapping_peaks(features, team_peaks)
overlapping_peaks
...
get_promoter_regions <- function(gencode_gr, biotype, upstream = 3000, downstream = 3000) {
  ...
  genes <- genes[which(genes$gene_type == "protein_coding")]
...
promoter_regions <- get_promoter_regions(gtf, "protein-coding")
...
num_overlaps <- count_peaks_per_feature(promoter_regions, team_peaks)
table(num_overlaps)
```
