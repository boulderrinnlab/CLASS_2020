...
num_peaks <- count_peaks_per_feature(features, peak_list, type = "counts")
non_zero_peak_list <- peak_list[which(sapply(peak_list, length)) > 0 ]
peak_list[which(length(peak_list) > 0)]

x <- sapply(peak_list, length)
...
# A: A lot! 17,000+20,000 promoters x ~190 TFs
...
shuffle_peaks <- function (peak_gr, genome) {
  ...
  #Issue here beause don't have peak_gr
