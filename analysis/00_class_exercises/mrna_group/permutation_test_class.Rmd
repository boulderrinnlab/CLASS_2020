...
#How to filter zero consensus peak TFS???
...
shuffle_peaks <- function (peak_gr, genome) {
  ...
  chrLens <- genome[names(seqlengths(region_set)),"size"]
  #make a vector of the sequence names (chromosomes) in peak_set
  nn <- as.vector(seqnames(region_set))
  ...
  res <- GRanges(seqnames = nn[ii], ranges = IRanges(ss, width = w[ii]), 
                 strand = "*")
  #give bac the GRanges just made
  return(res)
}
...
peak_list_nulls <- ?
  ...
  #What are we trying to make here? A null distribution? How?
...
for(i in 1:length(region_list)) {
  ...
  #Having issue here because haven't made peak_list_nulls
