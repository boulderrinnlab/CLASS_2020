...
# Let's retrieve some K562 RNA-seq

We'll include both Poly A selected and total RNA-seq to 

```{r}
samples <- read_csv("data/k562_rnaseq_bam_files.csv")
...
expression$file_accession <- gsub("rpk.|.bam", "", expression$sample)
exp <- merge(expression, samples)

write_csv(exp, "results/K562_expression.csv")
...
g <- ggplot(peak_occurrence_df, aes(y = log2(tpm +s), x = number_of_dbp))
g + geom_point(data = peak_occurrence_df %>% filter(tpm < 0.001), 
               shape = 17, alpha = 0.7) +
  geom_smooth(method = 'gam', formula = y ~ s(x, bs = "cs"),
              color = "#424242") +
