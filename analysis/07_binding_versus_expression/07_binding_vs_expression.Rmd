...
peak_occurrence_df <- peak_occurrence_df[ -c(8)]
...
g <- ggplot(peak_occurrence_df, aes(y = log2(tpm), x = number_of_dbp, color = gene_type))
g + 
...
```{r, message=FALSE, warning=FALSE}
g <- ggplot(peak_occurrence_df, aes(y = log2(tpm), x = number_of_dbp))
g + 
  geom_point(data = peak_occurrence_df %>% filter(tpm < 0.001), alpha = 0.2) +
  geom_smooth() + 
  scale_y_continuous(expand = c(0,0)) + 
