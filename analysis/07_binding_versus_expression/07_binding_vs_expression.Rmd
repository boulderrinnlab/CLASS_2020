...
# Our strategy will be to keep adding variables to our observations (promoters). We can then plot from this data frame as it grows. 
```{r}
peak_occurrence_df <- read_csv("../01_consensus_peaks/results/peak_occurence_dataframe.csv")
expression_occurrence_df <- read_csv("results/peak_tpm_occurence_df.csv")
...
peak_occurrence_df <- peak_occurrence_df[ -c(7)]
...
write_csv(peak_occurrence_df, "results/peak_occurence_df_exp_added.csv")
...
g <- ggplot(peak_occurrence_df, aes(y = log10(tpm), x = number_of_dbp, color = gene_type))
g + 
