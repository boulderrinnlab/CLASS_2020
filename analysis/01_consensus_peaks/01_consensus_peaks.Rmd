...
```{r}
# TODO: consider moving to a setup file.
...
plot_replicate_peaks <- function(dbps, region, bw_axis_limits,
                                 gencode_gm = "../util/gencode_genemodel_for_Gviz.csv") {
  
  if(length(region) > 1) stop("Please plot only one region at a time")
  # Retrieve gencode gene model
  gencode_gm <- read_csv(gencode_gm)
  ...
  # Read in peaks 
  peak_files <- list.files("/Shares/rinn_class/data/k562_chip/results/bwa/mergedLibrary/macs/broadPeak/", 
                                     full.names = T)
  peak_files <- peak_files[grep(dbps, peak_files)]
  ...
  # Read in bigwigs 
  bws <- list.files("/Shares/rinn_class/data/k562_chip/results/bwa/mergedLibrary/bigwig/", 
                    full.names = T)
  bws <- bws[grep(dbps, bws)]
...
region <- GRanges(seqnames = "chr20",
                  IRanges(start = 53609523,
                          end = 53896205))
rfx1_lgpeak <- plot_replicate_peaks("RFX1", region, bw_axis_limits = c(0,0.6))

aff1peak <- plot_replicate_peaks("AFF1", region, bw_axis_limits = c(0,0.6))
