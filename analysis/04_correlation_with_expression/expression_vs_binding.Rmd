...
```{r converting_read_counts_to_TPM}
...
```{r getting_list_of_consensus_peaks}
...
```{r getting_promoters_from_gencode}
...
```{r determining_the_number_of_peaks_in_each_promoter}
...
```{r creating_a_data_frame_of_genes_peak_counts_and_gene_types}
...
```{r plotting_expression_vs_binding_events}
...
```{r plotting_expression_vs_binding_events_but_now_there_are_ghosts_that_have_no_epression_and_the_messy_dot_situation_is_gone}
...
```{r plotting_frequency_of_promoters_with_each_peak_density_and_histograms_for_peaks_of_different_biotypes}
...
```{r plotting_peak_occurances_for_lowly_vs_highly_bound_promoter_regions}
