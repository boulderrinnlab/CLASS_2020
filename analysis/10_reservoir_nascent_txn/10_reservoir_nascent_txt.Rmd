...
counts_table <- counts$counts %>%
  as.data.frame() %>%
  rownames_to_column("gene_id")
write_csv(counts_table, "results/k562_nascent_raw_counts.csv")
...
# Write out sample tpm
expression_table  <- expression %>%
  dplyr::select(sample, gene_id, tpm) %>%
  pivot_wider(names_from = sample, values_from = tpm)
write_csv(expression_table, "results/k562_nascent_tpm_per_sample.csv")
...
tpm <- read_csv("results/k562_nascent_tpm.csv")
```
