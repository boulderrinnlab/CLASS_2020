...
nascent_exp <- nascent_exp %>% 
  dplyr::select(gene_id, tpm) %>%
  column_to_rownames("gene_id") %>%
  as.matrix()
rnaseq_exp <- rnaseq_exp %>% 
  dplyr::select(gene_id, tpm) %>%
  column_to_rownames("gene_id") %>%
  as.matrix()
# Match the order
nascent_exp <- nascent_exp[rownames(dbp_occurence_matrix),]
rnaseq_exp <- rnaseq_exp[rownames(dbp_occurence_matrix),]
stopifnot(all(rownames(nascent_exp) == rownames(dbp_occurence_matrix)))
stopifnot(all(rownames(rnaseq_exp) == rownames(dbp_occurence_matrix)))
...
names(exp_df)[2] <- "tpm"
# exp_df$expressed <- FALSE
# exp_df[which(exp_df$tpm > 0.01),"expressed"] <- TRUE

occ_df <- dbp_occurence_matrix %>%
  as.data.frame() %>%
  rownames_to_column("gene_id")
occ_df <- merge(exp_df %>% dplyr::select(gene_id, tpm), occ_df) %>%
  column_to_rownames("gene_id")
...
# Convert to log10
occ_df$tpm <- log10(occ_df$tpm + 0.05)
...
h2o.partialPlot(h2o.fit1, data = train.h2o, cols = "BCOR")
h2o.partialPlot(h2o.fit1, data = train.h2o, cols = "NRF1")
h2o.partialPlot(h2o.fit1, data = train.h2o, cols = "AFF1")
```

```{r}
reservoirs <- read_csv("../05_rnaseq_expression/results/k562_reservoir_promoters_info.csv")
res_na_tpm <- nascent_exp[which(nascent_exp$gene_id %in% reservoirs$gene_id),]
table(res_na_tpm$tpm > 0)
reservoir_indices <- which(rownames(exp_test) %in% reservoirs$gene_id)
non_expressed_indices <- which(rownames(exp_test) %in% res_na_tpm$gene_id[which(res_na_tpm$tpm == 0)])
expressed_indices <- which(rownames(exp_test) %in% res_na_tpm$gene_id[which(res_na_tpm$tpm > 0)])

local_obs <- exp_test[c(1,2), ]
explainer <- lime(exp_train, h2o.fit1)
explanation <- lime::explain(local_obs, explainer, n_labels = 2,n_features = 5)
plot_features(explanation)
exp_test[c(1,2), 1]
rownames(exp_test)[1:2]

table(exp_train$expressed)
...
lmdf <- data.frame("term" = character(),
                   ...
                   "p.value" = numeric(),
                   "dbp" = character())
for(i in 2:ncol(occ_df)) {
  t_lmdf <- tidy(lm(occ_df$log10_tpm ~ occ_df[,i]))
...
g <- ggplot(lmdf_slopes, aes(x = estimate, y = -log10(padj), label = dbp))
g + geom_point() +
  geom_text() + 
